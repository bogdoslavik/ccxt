...
const OUTPUT_PATH = process.env.BIDSASKS_SPREAD_OUTPUT ?? 'bidsasks-spread.png';
const LOAD_PROGRESS_STEPS = 20;
const POINT_RADIUS = 2;
const MAX_LINE_GAP_MS = 2000;
const MIN_SYMBOL_DELTA_PERCENT = 2;
...
async function main (): Promise<void> {
    ...
    try {
        const collection = mongoClient.db ('arb').collection<ArbDocument> ('perps');
        ...
        for await (const doc of cursor) {
            ...
            if (totalDocs > 0) {
                ...
                if (currentStep > lastLoggedStep) {
                    ...
                    console.log (`Loading arb rows... ${processed}/${totalDocs} (${percent}%)`);
                    lastLoggedStep = currentStep;
                }
            } else if (processed % 1000 === 0) {
                console.log (`Loading arb rows... ${processed} processed`);
            }
        }
        if (totalDocs > 0 && processed < totalDocs) {
            console.log (`Loading arb rows... ${processed}/${totalDocs} (100.0%)`);
        }
        if (documents.length === 0) {
            console.log ('No arb documents found.');
            return;
        }

        let rows = documents
            .map ((doc) => ({
                symbol: getBaseSymbol (doc.symbol ?? 'UNKNOWN'),
                timestamp: normalizeTimestamp (doc.timestamp),
                deltaWFees: toPercentNumber (doc.deltaWFees),
            }))
            .filter ((row) => Number.isFinite (row.timestamp) && Number.isFinite (row.deltaWFees));

        if (rows.length === 0) {
            console.log ('No valid rows with timestamps or deltaWFees.');
            return;
        }

        // convert to percents for plotting
        rows = rows.map ((row) => ({ ...row, deltaWFees: (row.deltaWFees as number) * 100 }));

        const symbolMax = new Map<string, number> ();
        rows.forEach ((row) => {
            const currentMax = symbolMax.get (row.symbol) ?? -Infinity;
            symbolMax.set (row.symbol, Math.max (currentMax, row.deltaWFees as number));
        });
        ...
        if (rows.length === 0) {
            console.log (`No symbols exceeded ${MIN_SYMBOL_DELTA_PERCENT.toFixed (2)}% delta.`);
            return;
        }

        const minTime = rows[0].timestamp as number;
        const maxTime = rows[rows.length - 1].timestamp as number;
        const timeRange = Math.max (1, maxTime - minTime);
        const maxDeltaValue = rows.reduce ((acc, row) => Math.max (acc, Math.max (0, row.deltaWFees as number)), 0);
        ...
        for (let val = 0; val <= yMax + 1e-9; val += Y_STEP_PERCENT) {
            ...
            const isWholePercent = Math.abs (val - Math.round (val)) < 1e-9;
            ctx.strokeStyle = isWholePercent ? GRID_BOLD_COLOR : ((Math.abs (val % (Y_STEP_PERCENT * 5)) < 1e-9) ? GRID_BOLD_COLOR : GRID_COLOR);
            ctx.beginPath ();
            ctx.moveTo (chartLeft, y);
            ctx.lineTo (chartRight, y);
            ctx.stroke ();
            if (isWholePercent) { // show labels only at whole percents
                ctx.fillStyle = TEXT_COLOR;
                ctx.font = `${AXIS_TEXT_SIZE}px Inter, Arial, sans-serif`;
                ctx.textAlign = 'right';
                ctx.textBaseline = 'middle';
                ctx.fillText (`${val.toFixed (0)}%`, chartLeft - 20, y);
                ctx.textAlign = 'left';
                ctx.fillText (`${val.toFixed (0)}%`, chartRight + 20, y);
            }
        }
        ...
        rows.forEach ((row) => {
            ...
            seriesMap.get (row.symbol).points.push ({
                x: timeToX (row.timestamp as number),
                y: valueToY (row.deltaWFees as number),
                value: row.deltaWFees as number,
